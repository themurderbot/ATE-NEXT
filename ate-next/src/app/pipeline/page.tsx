...
type Request = {
  ...
  total_amount: number; devices_count: number; created_at: string
...
export default function PipelinePage() {
  ...
  async function load() {
    setLoading(true)
    const { data } = await supabase.from('requests').select('*, clients(company_name), properties(property_name)').order('created_at', { ascending: false })
  ...
  if (loading) return (
    <div dir={dir} style={{ fontFamily: 'Tajawal, sans-serif', minHeight: '100vh', background: '#f7f8fc' }}>
      ...
      <div style={{ padding: '60px', textAlign: 'center', color: '#a0aec0' }}>⏳</div>
    </div>
  )
    ...
    <div dir={dir} style={{ fontFamily: 'Tajawal, Arial, sans-serif', minHeight: '100vh', background: '#f7f8fc' }}>
      ...
      <div style={{ padding: '24px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
          <div>
            <div style={{ fontSize: '20px', fontWeight: 900, color: '#1a202c' }}>🔄 {t('Pipeline الطلبات','Requests Pipeline')}</div>
            <div style={{ fontSize: '12px', color: '#718096', marginTop: '2px' }}>
              {requests.length} {t('طلب','requests')} · {totalValue.toLocaleString()} {t('د','AED')} · {certified} {t('شهادة','certified')}
            </div>
          </div>
            ...
            return (
              ...
              >
                <div style={{ padding: '12px 14px', borderBottom: `3px solid ${stage.color}`, background: `${stage.color}08` }}>
                  ...
                  {stageValue > 0 && <div style={{ fontSize: '11px', color: stage.color, marginTop: '4px', fontWeight: 600 }}>{stageValue.toLocaleString()} {t('د','AED')}</div>}
                </div>

                <div style={{ padding: '10px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  {cards.map(r => (
                    <div key={r.id} draggable onDragStart={() => setDragging(r.id)} onDragEnd={() => setDragging(null)}
                    ...
                    >
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                        ...
                        <span style={{ fontSize: '10px', color: '#a0aec0' }}>{r.devices_count} {t('جهاز','dev')}</span>
                      </div>
                      <div style={{ display: 'flex', gap: '4px' }}>
                        {prevStage && (
                          <button onClick={() => moveStage(r.id, prevStage.id)} style={{ flex: 1, padding: '4px 5px', border: `1px solid ${prevStage.color}44`, background: `${prevStage.color}10`, color: prevStage.color, borderRadius: '5px', fontSize: '9px', fontWeight: 700, cursor: 'pointer', fontFamily: 'Tajawal,sans-serif' }}>
                            ← {prevStage.icon}
                          </button>
                        )}
                        {nextStage ? (
                          <button onClick={() => moveStage(r.id, nextStage.id)} style={{ flex: 2, padding: '4px 5px', border: `1px solid ${nextStage.color}44`, background: `${nextStage.color}15`, color: nextStage.color, borderRadius: '5px', fontSize: '9px', fontWeight: 700, cursor: 'pointer', fontFamily: 'Tajawal,sans-serif' }}>
                            {nextStage.icon} {nextStage.label} →
                          </button>
                        ) : (
                          <div style={{ flex: 2, padding: '4px', textAlign: 'center', fontSize: '9px', color: '#38a169', fontWeight: 700 }}>✅ {t('مكتمل','Done')}</div>
                        )}
                      </div>
                    </div>
                  ))}
